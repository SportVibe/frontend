...
const ShoppingCart = () => {
  ...
  const calculateLocalSubtotal = () => {
    ...
    return cartItems.cart.reduce((total, item) => total + item.price * item.quantity, 0);
  };
  ...
  const handleRemoveFromCart = async (productId, productSize) => {
    ...
    const updateCart = cartItems?.cart.filter((product) => {
      if (
        (Number(product.id) === Number(productId) && product.size !== productSize.toString()) ||
    ...
    if (userId) {
      const result = await axios.delete(
        `${API_URL}/deleteShoppingProduct?userId=${userId}&productId=${productId}&productSize=${productSize}`
      );
    }
        ...
        <div>
          ...
          {localSubtotal <= 80 && (
            <p className="text-success">
              ¡Ya casi! Para envío gratis a ciudades principales, agrega ${80 - localSubtotal} más.
            </p>
          )}
        </div>
        <div>
